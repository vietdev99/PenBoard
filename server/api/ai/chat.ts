 ...
 *
 * When `insideProject` is true, files are saved under `.penboard-tmp/` in the
...
async function saveAttachmentsToTempFiles(
  ...
  if (insideProject) {
    ...
    const baseDir = join(process.cwd(), '.penboard-tmp')
    mkdirSync(baseDir, { recursive: true, mode: 0o700 })
    chmodSync(baseDir, 0o700)
    tempDir = await mkdtemp(join(baseDir, 'attach-'))
  } else {
    tempDir = await mkdtemp(join(tmpdir(), 'penboard-attach-'))
  }
  const files: string[] = []
